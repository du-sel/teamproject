...
public interface PurchaseListRepository extends JpaRepository<PurchaseListVO, PurchaseId> {
	...
	@Query(value = "SELECT pl.p_name, COUNT(pl.bak_p_id) AS quantity, SUM(pl.price) AS total FROM purchaselist pl, products p WHERE pl.c_id=:id and p.p_id=pl.bak_p_id GROUP BY pl.bak_p_id ORDER BY quantity DESC, total DESC LIMIT 5", nativeQuery = true)
